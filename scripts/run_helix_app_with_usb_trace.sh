...
TRACE_DIR="${TRACE_DIR:-/tmp/helix-usb-trace}"
TRACE_APP_DIR="${TRACE_APP_DIR:-$TRACE_DIR/Helix Stadium Traced.app}"
TRACE_LIB="${TRACE_LIB:-$TRACE_DIR/libusb_trace_interpose.dylib}"
TRACE_LOG="${TRACE_LOG:-$TRACE_DIR/helix_usb_trace.log}"
...
clang \
  ...
  -Wextra \
  -o "$TRACE_LIB" \
  "$ROOT_DIR/scripts/libusb_trace_interpose.c"
...
HELIX_USB_TRACE_LOG="$TRACE_LOG" \
DYLD_INSERT_LIBRARIES="$TRACE_LIB" \
"$APP_BIN" >/dev/null 2>&1 &

APP_PID=$!
echo "PID=$APP_PID"
echo "Use 'tail -f \"$TRACE_LOG\"' to inspect the USB trace."
