...
- DSP meters use per-model `usage` values from the modeldefs bundle and a **70 usage** reference cap per path. When Auto-cab is enabled, amp insertion checks also include the linked cab usage from the same bundle.
